#!/bin/bash
#
# Build the bismark-agent-mock Docker image
#
# Usage: ./build-mock.sh
#
...
echo "  docker run --rm -e BISMARK_TASK_ID=test-1 $IMAGE_NAME"
echo ""
echo "To test with custom interval (faster):"
echo "  docker run --rm -e BISMARK_TASK_ID=test-1 -e MOCK_EVENT_INTERVAL_MS=500 $IMAGE_NAME"

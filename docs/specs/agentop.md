# AgentOp Specification

Desktop application for monitoring and managing Claude Code agents

## Overview

AgentOp is a desktop application for monitoring and managing Claude Code agents. It provides a unified interface to:
...
- **Mechanism:** Configure a StopHook in Claude Code that communicates with the AgentOp app when agent requires input
...
- **Approach:** In-app configurability (initially can seed from bootstrap-claude)
- **Configuration per workspace:** name, directory, theme/styling
- **Storage:** App maintains its own config in ~/.agent-operator/
...
- [ ] StopHook script connects to Unix socket at ~/.agent-operator/sockets/agent-{id}.sock
